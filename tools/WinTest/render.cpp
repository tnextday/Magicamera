...
{
    this->resize(640, 480);
    if (!eglCreateSurface(this->winId(), g_eglSurface, g_eglDisplay))
        return;
...
{
    g_MagicMain.resize(e->size().width(), e->size().height());
}

void Render::mouseMoveEvent( QMouseEvent * e)
{
    g_MagicMain.onTouchDrag(e->x(), e->y());
}

void Render::mouseReleaseEvent( QMouseEvent * e)
{
    g_MagicMain.onTouchUp(e->x(), e->y());
}

void Render::mousePressEvent( QMouseEvent * e)
{
    g_MagicMain.onTouchDown(e->x(), e->y());
}
...
import com.funny.magicamera.utils.IntMap.Entry;
...
public class SoundEngine {
    ...
    public void setSoundVolume(Float volume) {
    	...
    	soundsVolume = volume;
    	for (Entry<MediaPlayer> each : soundsMap.entries())
    	{
    		each.value.setVolume(soundsVolume, soundsVolume);
    	}
    }
...
//			mp.prepareAsync();
	...
	public void releaseSound(int resId)
	{
		MediaPlayer mp = null;
		synchronized(soundsMap) {
			mp = soundsMap.get(resId);
	...
	public void releaseAllSounds() {
		...
		for(Entry<MediaPlayer> mp : soundsMap.entries()) {
			mp.value.release();
		}
